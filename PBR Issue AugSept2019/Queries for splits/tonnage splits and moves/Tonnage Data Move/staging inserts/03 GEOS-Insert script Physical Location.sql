...
When        Who                 What
----------  ------------------- ----------------------------------------
03/12/20    Tom Karasch         modified to use with Solid Waste Program
***********************************************************************/
...
FROM [GovOnline_GEOS].[dbo].[SYS_PHYSICAL_LOCATION];
IF 'EPDMIG ' <>
...
SELECT @rid_counter_start + ROW_NUMBER() OVER(ORDER BY
    (SELECT 1)) AS [LOCATION_RID],
       ...
       [SPL].[GEO_COORDINATE_RID] AS [GEO_COORDINATE_RID],
       'A' AS [STATUS_CD],
