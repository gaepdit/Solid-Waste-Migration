...
04/14/2020  Tom Karasch         Modified for PROD
***********************************************************************/
...
FROM [LEMIR_Stage].[dbo].[$1_GEOS_FAC_FACILITY] AS [FF]
     RIGHT JOIN [LEMIR_Stage].[dbo].[$1_GEOS_SYS_ADDRESS] AS [SA] ON [FF].[FACILITY_ID_REF] = [SA].[FACILITY_ID_REF]
WHERE [SA].[FACILITY_ID_REF] IN('141-013D(SL)', '076-020D(MSWL)', '137-007D(SL)')   --'028-039D(SL)', '047-014D(SL)', ;  
...
WHERE [FL].[FACILITY_ID_REF] IN('141-013D(SL)', '076-020D(MSWL)', '137-007D(SL)')   --'028-039D(SL)', '047-014D(SL)', ;   
