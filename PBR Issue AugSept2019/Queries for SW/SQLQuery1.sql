...
--
    --  isnull(SUBSTRING(
    --(SELECT DISTINCT
    --        (', '+[su].[FIRST_NAME]+' '+[su].[LAST_NAME]) AS [text()]
    -- FROM [GovOnline_LEMIR].[dbo].[FAC_ASSIGNMENT] AS [fa]
    --      JOIN [GovOnline_LEMIR].[dbo].[SEC_USER] AS [su] ON [fa].[USER_RID] = [su].[USER_RID]
    -- WHERE [fa].[FACILITY_RID] = [ff].[Facility_Rid] FOR XML PATH('')), 2, 1000), '') AS [Geologists],