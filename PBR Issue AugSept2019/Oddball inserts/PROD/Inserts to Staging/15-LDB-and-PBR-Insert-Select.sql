...
/***********************************************************************
Author:      Tom Karasch
Overview:    Env. Program Location insert script
             for SW from LDB
...
--
SET @rid_counter_start=588768; -- current Max RID - 5/1/20    PROD
--
IF 'EPDMIG SW OB' =
    (SELECT [CREATED_BY]
  ...
  END
  --
--INSERT INTO [LEMIR_Stage_OB].[dbo].[FAC_ENV_PROGRAM_LOC]
       ...
       convert(DATETIME,
               CASE
                 WHEN [FEP].[TYPE_RID] IN(10041, 10042, 10043, 10044, 10045, 10046, 10047, 10048, 10049, 10050, 10051, 10052, 10053)
                   THEN [FEP].[PROGRAM_DETAIL].value('(/DynamicFormValue/SolidWaste/datafield[(id/text())[1]="txtPermitIssueDate"]/value/text())[1]', N'varchar(20)')
                 WHEN [FEP].[TYPE_RID] IN(10054, 10055, 10056, 10057)
                   THEN [FEP].[PROGRAM_DETAIL].value('(/DynamicFormValue/SolidWaste/datafield[(id/text())[1]="txtFacilityStartDate"]/value/text())[1]', N'varchar(20)')
                 ELSE ''
               END) AS [EFF_DATE],
                  --
       convert(DATETIME,
               CASE
                 WHEN [FEP].[TYPE_RID] IN(10041, 10042, 10043, 10044, 10045, 10046, 10047, 10048, 10049, 10050, 10051, 10052, 10053)
                   THEN [FEP].[PROGRAM_DETAIL].value('(/DynamicFormValue/SolidWaste/datafield[(id/text())[1]="txtPermitReviewDueDate"]/value/text())[1]', N'varchar(20)')
                 WHEN [FEP].[TYPE_RID] IN(10054, 10055, 10056, 10057)
                   THEN [FEP].[PROGRAM_DETAIL].value('(/DynamicFormValue/SolidWaste/datafield[(id/text())[1]="txtFacilityEndDate"]/value/text())[1]', N'varchar(20)')
                 WHEN [FEP].[TYPE_RID] = 10056
                   THEN [FEP].[PROGRAM_DETAIL].value('(/DynamicFormValue/SolidWaste/datafield[(id/text())[1]="txtClosureAcknowledgedDate"]/value/text())[1]', N'varchar(20)')
                 ELSE ''
               END) AS [EXP_DATE],
      --
       ...
       [FF].[FACILITY_ID_REF]
FROM [LEMIR_Stage_OB].[dbo].[FAC_ENV_PROGRAM] AS [FEP]
     JOIN [LEMIR_Stage_OB].[dbo].[FAC_FACILITY] AS [FF] ON [FF].[FACILITY_RID] = [FEP].[FACILITY_RID]
     RIGHT JOIN [LEMIR_Stage_OB].[dbo].[SYS_PHYSICAL_LOCATION] AS [SPL] ON [SPL].[FACILITY_ID_REF] = [FF].[FACILITY_ID_REF]
