...
set @rid_counter_start = 15048; -- Max SYS_DISPOSAL_HISTORY_RID -- 5/1/20   PROD
--
--IF 'EPDMIG SWT' =
--    (SELECT [CREATED_BY]
--     FROM [LEMIR_Stage_OB].[dbo].[SYS_DISPOSAL_HISTORY]
...
--  END
  --
--INSERT INTO [LEMIR_Stage_OB].[dbo].[SYS_DISPOSAL_HISTORY]
...
SELECT @rid_counter_start + ROW_NUMBER() OVER(ORDER BY
    (SELECT 1)) AS [SYS_DISPOSAL_HISTORY_RID],
       ...
       @created_by_string AS [REVISED_BY],
       getdate() AS [BATCH_TIMESTAMP]
FROM [LEMIR_Stage_OB].[dbo].[SYS_DISPOSAL] AS [SD]
