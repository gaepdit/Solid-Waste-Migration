...
SELECT @rid_counter_start + ROW_NUMBER() OVER(ORDER BY
    (SELECT 1)) AS [CONTACT_RID],
       isnull(right([LC].[Owner/ContactName], (charindex(' ', reverse([LC].[Owner/ContactName]), 1))),'') AS [LAST_NAME],
       isnull(left([LC].[Owner/ContactName], (charindex(' ', [LC].[Owner/ContactName], 1))),'') AS [FIRST_NAME],
       '' AS [MIDDLE_INITIAL],
