...
FROM [LEMIR_Stage_OB].[dbo].[FAC_FACILITY] AS [FF]
     FULL OUTER JOIN [LEMIR_Stage_OB].[dbo].[$EI_insert_update] AS [UI] ON [FF].[FACILITY_ID_REF] = [UI].[MainPermitNumber]
     FULL OUTER JOIN [LEMIR_Stage_OB].[dbo].[$EI_insert_update] AS [UI2] ON [FF].[FACILITY_ID_REF] = [UI2].[analysis hist notes]
     FULL OUTER JOIN [LEMIR_Stage_OB].[dbo].[$EI_insert_update] AS [UI3] ON [FF].[FACILITY_RID] = [UI2].[LEMIR ID for Update]
WHERE [UI].[GWM] = 'Y'
      OR [UI2].[GWM] = 'Y'
      OR [UI3].[GWM] = 'Y'
...
INTO [#temp4]
FROM [#temp3] AS [F]
     LEFT JOIN [LEMIR_Stage_OB].[dbo].[FAC_FACILITY] AS [FF] ON [F].[FACILITY_RID] = [FF].[FACILITY_RID]
...
FROM [#temp4] AS [F]
     ...
     LEFT JOIN [LEMIR_Stage_OB].[dbo].[FAC_FACILITY] AS [FF] ON [FF].[FACILITY_RID] = [F].[Stage_FACILITY_RID]
