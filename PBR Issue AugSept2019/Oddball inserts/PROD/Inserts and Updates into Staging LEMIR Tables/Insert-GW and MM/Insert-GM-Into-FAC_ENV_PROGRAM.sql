...
FROM [LEMIR_Stage_OB].[dbo].[$EI_insert_update] AS [UI]
      left JOIN [LEMIR_Stage_OB].[dbo].[EI_GW_MM_XML] AS [M] ON [UI].[MainPermitNumber] = [M].[PermitNumber]
      left JOIN [LEMIR_Stage_OB].[dbo].[FAC_FACILITY] AS [FF] ON [UI].[MainPermitNumber] = [FF].[FACILITY_ID_REF]
