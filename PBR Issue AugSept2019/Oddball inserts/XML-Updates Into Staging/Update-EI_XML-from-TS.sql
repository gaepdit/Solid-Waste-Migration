...
UPDATE [dbo].[EI_TYPE]
   SET [dbo].[EI_TYPE].[LEMIR_XML]=
    ...
    (SELECT DISTINCT
     ...
     FROM [LEMIR_Stage].[dbo].[EI_TS] AS [C]
    ...
    (SELECT DISTINCT
     ...
     FROM [LEMIR_Stage].[dbo].[EI_TS] AS [B]
     ...
     FROM [dbo].[EI_TYPE] AS [EI]
          INNER JOIN [dbo].[EI_TS] AS [C] ON [EI].[PermitNumber] = [C].[PermitNumber]
     WHERE [EI].[PermitNumber] = [E].[PermitNumber] FOR XML PATH('SolidWaste'), ROOT('DynamicFormValue'))
     --EI Where clause
FROM [dbo].[EI_TYPE] AS [E]
     INNER JOIN [dbo].[EI_TS] AS [LI] ON [E].[PermitNumber] = [LI].[PermitNumber]
WHERE [E].[LEMIR_EI_CD] = 'TS'
