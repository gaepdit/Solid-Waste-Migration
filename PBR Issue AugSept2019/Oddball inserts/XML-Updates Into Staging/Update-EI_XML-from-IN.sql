...
UPDATE [dbo].[EI_TYPE]
   SET [dbo].[EI_TYPE].[LEMIR_XML]=
    ...
    (SELECT DISTINCT
     ...
     FROM [LEMIR_Stage].[dbo].[EI_IN] AS [C]
    ...
    (SELECT DISTINCT
     ...
     (SELECT DISTINCT
            [C].[ddlInertLandfillType_ID] AS [id],
            [C].[ddlInertLandfillType_VAL] AS [value],
            [C].[ddlInertLandfillType_TAG] AS [tag],
            [C].[ddlInertLandfillType_VIS] AS [visible],
            [C].[ddlInertLandfillType_HIS] AS [history],
            [C].[ddlInertLandfillType_DES] AS [description],
            [C].[ddlInertLandfillType_COM] AS [comment]
     FROM [LEMIR_Stage].[dbo].[EI_IN] AS [C]
          INNER JOIN [LEMIR_Stage].[dbo].[EI_TYPE] AS [I] ON [C].[PermitNumber] = [I].[PermitNumber]
     WHERE [C].[PermitNumber] = [EI].[PermitNumber] FOR XML PATH('dataField'), TYPE) AS [*],
    ...
    (SELECT DISTINCT
     ...
     FROM [LEMIR_Stage].[dbo].[EI_IN] AS [B]
    ...
    (SELECT DISTINCT
     ...
     FROM [dbo].[EI_TYPE] AS [EI]
          INNER JOIN [dbo].[EI_IN] AS [C] ON [EI].[PermitNumber] = [C].[PermitNumber]
     WHERE [EI].[PermitNumber] = [E].[PermitNumber] FOR XML PATH('SolidWaste'), ROOT('DynamicFormValue'))
     --EI Where clause
FROM [dbo].[EI_TYPE] AS [E]
     INNER JOIN [dbo].[EI_IN] AS [LI] ON [E].[PermitNumber] = [LI].[PermitNumber]
WHERE [E].[LEMIR_EI_CD] = 'IN'
