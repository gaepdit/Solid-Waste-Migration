...
SELECT [MF].[MainPermitNumber] AS [PermitNumber],
       ...
       [MF].[MainPermitNumber] AS [txtPermitNumber_VAL],
       'Permit Number' AS [txtPermitNumber_TAG],
       'true' AS [txtPermitNumber_VIS],
       [MF].[MainPermitNumber]+'|'+convert(VARCHAR(50), getdate(), 101)+' '+LTRIM(RIGHT(CONVERT(CHAR(20), GETDATE(), 22), 11))+'|'+'EPDMIG SW||' AS [txtPermitNumber_HIS],
       ...
       IIF([MF].[FacilityStartDate] IS null, '', convert(VARCHAR(50), [MF].[FacilityStartDate], 101)) AS [txtFacilityStartDate_VAL],
       'Facility Start Date' AS [txtFacilityStartDate_TAG],
       'true' AS [txtFacilityStartDate_VIS],
       IIF([MF].[FacilityStartDate] IS null, '', convert(VARCHAR(50), [MF].[FacilityStartDate], 101)+'|'+convert(VARCHAR(50), getdate(), 101)+' '+LTRIM(RIGHT(CONVERT(CHAR(20), GETDATE(), 22), 11))+'|'+'EPDMIG SW||') AS [txtFacilityStartDate_HIS],
       'Facility Start Date:' AS [txtFacilityStartDate_DES],
       '' AS [txtFacilityStartDate_COM],
       -- 
       'txtPBRApprovalDate' AS [txtPBRApprovalDate_ID],
       IIF([MF].[PermitIssueDate] IS null, '', convert(VARCHAR(50), cast([MF].[PermitIssueDate] AS DATETIME), 101)) AS [txtPBRApprovalDate_VAL],
       'PBR Approval Date' AS [txtPBRApprovalDate_TAG],
       'true' AS [txtPBRApprovalDate_VIS],
       IIF([MF].[PermitIssueDate] IS null, '', convert(VARCHAR(50), cast([MF].[PermitIssueDate] AS DATETIME), 101)+'|'+convert(VARCHAR(50), getdate(), 101)+' '+LTRIM(RIGHT(CONVERT(CHAR(20), GETDATE(), 22), 11))+'|'+'EPDMIG SW||') AS [txtPBRApprovalDate_HIS],
       ...
       isnull(convert(varchar(50),[MF].[DateCeasedAcceptingWaste],110), '')  AS [txtAcceptingWasteCeasedDate_VAL],
       'Date Ceased Accepting Waste' AS [txtAcceptingWasteCeasedDate_TAG],
       'true' AS [txtAcceptingWasteCeasedDate_VIS],
       isnull(convert(varchar(50),[MF].[DateCeasedAcceptingWaste],110)+'|'+convert(VARCHAR(50), getdate(), 101)+' '+LTRIM(RIGHT(CONVERT(CHAR(20), GETDATE(), 22), 11))+'|'+'EPDMIG SW||','') AS [txtAcceptingWasteCeasedDate_HIS],
       ...
       IIF([MF].[DateClosureCertificate Issued] IS null, '', convert(varchar(50),[MF].[DateClosureCertificate Issued],101)) AS [txtClosureAcknowledgedDate_VAL],
       'Closure Acknowledged Date' AS [txtClosureAcknowledgedDate_TAG],
       'true' AS [txtClosureAcknowledgedDate_VIS],
       IIF([MF].[DateClosureCertificate Issued] IS null, '', convert(varchar(50),[MF].[DateClosureCertificate Issued],101)+'|'+convert(VARCHAR(50), getdate(), 101)+' '+LTRIM(RIGHT(CONVERT(CHAR(20), GETDATE(), 22), 11))+'|'+'EPDMIG SW||') AS [txtClosureAcknowledgedDate_HIS],
       ...
       [MF].[TotalAcreage] AS [txtTotalAcreage_VAL],
       'Total Acreage (acres)' AS [txtTotalAcreage_TAG],
       'true' AS [txtTotalAcreage_VIS],
       [MF].[TotalAcreage] AS [txtTotalAcreage_HIS],
       'Total Acreage (acres):' AS [txtTotalAcreage_DES],
       '' AS [txtTotalAcreage_COM],
       --
       'txtWasteDescription' AS [txtWasteDescription_ID],
       [MF].[FacilityTypeDescription] AS [txtWasteDescription_VAL],
       'Waste Description' AS [txtWasteDescription_TAG],
       'true' AS [txtWasteDescription_VIS],
       [MF].[FacilityTypeDescription]+'|'+convert(VARCHAR(50), getdate(), 101)+' '+LTRIM(RIGHT(CONVERT(CHAR(20), GETDATE(), 22), 11))+'|'+'EPDMIG SW||' AS [txtWasteDescription_HIS],
...
--       LEFT JOIN [LEMIR_Stage_fixes].[dbo].[EI_TYPE] AS [EI] ON [MF].[PermitNumber] = [EI].[PermitNumber] 
