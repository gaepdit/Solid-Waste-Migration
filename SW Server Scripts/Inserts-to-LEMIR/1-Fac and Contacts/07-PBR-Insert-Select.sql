...
SELECT @rid_counter_start + ROW_NUMBER() OVER(ORDER BY
    (SELECT 1)) AS [TELEPHONIC_RID],
       '1' AS [PHONE_TYPE_RID],
       isnull([PC].[Owner/ContactAreaCode], [PC].[FacilityManagerAreaCode]) AS [PHONE_AREA_CODE],
       isnull([PC].[Owner/ContactPhoneNumber], [PC].[FacilityManagerTelephoneNumber]) AS [PHONE_NO],
       '' AS [PHONE_EXTENSION],
       'A' AS [STATUS_CD],
